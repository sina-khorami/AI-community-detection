...
bool MSCD_LFK::Run(const ds::Graph & g,
	...
	for (int p=0; p<ps.size(); ++p) {
		...
		bool grow_com = true, merge_com = true, done;
		while (grow_com) {
			...
			if (communities.GetNbCommunities() == 0) { // Grow initial communities from seeds
				...
				std::set<long>::iterator it;
				long i, n;
				for (i=0; i<al.size(); ++i) seeds.insert(i);
				...
				while (!seeds.empty()) {
					...
					// Insert in the set of communities to check
					check_coms.insert(communities.GetNbCommunities()-1);
#ifdef __DEBUG__
					double dkin, dktot;
...
#endif
					// Update seeds
					for (i=0; i<com.size(); ++i) seeds.erase(com[i]);
				}
				...
				for (long i=0, j, s, ds; i<coms.size(); ++i) {
                    ...
                    s = coms[i].size();
					if (this->grow(al, ps[p], coms[i], kin[i], ktot[i], coms_nbs[i])) {
						merge_com |= true;
						check_coms.insert(i);
#ifdef __DEBUG__
						double dkin, dktot;
			...
			do {
				...
				while (!check_coms.empty()) {
					// Take the first one
					c1 = *check_coms.begin();
					// Remove it from the set
					check_coms.erase(check_coms.begin());
					// Set bits for c1
					...
					while (c2 < coms.size()) {
							 ...
							 ((static_cast<double>(n)) >= this->merge_th*coms[c2].size())))
						{
							...
							it = check_coms.find(coms.size());
							if (it != check_coms.end()) {
								check_coms.erase(it);
								check_coms.insert(c2);
							}
			...
			while(!done); // While communities can potentially be merged
...
bool MSCD_LFK::grow(const ds::Graph::adjlist & al, double a, std::vector<long> & com, double & kin, double & ktot, std::set<long> & nbs) {
	...
	for (std::set<long>::iterator it = nbs.begin(); it != nbs.end(); ++it) {
		...
		knin *= 2.;
		nbs_q.insert(std::make_pair(*it,knin/std::pow(static_cast<double>(kntot),a)));
	}
	// Take all neighbours in order
	long j, n, x;
	preal fg;
	std::set<std::pair<long,preal>,great_sim>::iterator it;
